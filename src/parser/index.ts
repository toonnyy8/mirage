...
type typeCFSM = Array<typeCFSMElem>

// closurer 接收 grammar 後產生該語法的 closure 生成器
const genCFSM = (grammar: Grammar, startRule: number) => {
    let genClosure = closurer(grammar)

    // closure 生成器接收 未分析的ClosureElem集合 與 已分析的ClosureElem集合 之後會生成一整個 closure
    let _genCFSM = (unsolved: typeClosureStack, solved: typeClosureStack, cfsm: typeCFSM): [typeCFSM, typeClosureStack] => {
        ...
        if (unsolved.length != 0) {
            ...
            ({ cfsmElem, unsolveClosures } =
                Object.keys(unsolveClosures)
                    ...
                    .reduce((prev, key) => {
                        if (key == "undefined") {
                            console.log(prev.unsolveClosures[key])
                        }
            ...
            ({ cfsmElem, newUnsolved } = Object.keys(unsolveClosures)
                ...
                .reduce((prev, curr) => {
                    ...
                    if (state != -1) {
                        ...
                        console.log(prev.closureStack.length)
                        console.log(curr.value)
                        console.log(prev.closureStack)
